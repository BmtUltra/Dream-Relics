...
public class SetCountFromConfig extends LootItemConditionalFunction {
    ...
    private SetCountFromConfig(LootItemCondition[] conditions, CountConfig countConfig) {
        super(conditions);
    ...
    public static class Serializer extends LootItemConditionalFunction.Serializer<SetCountFromConfig> {
        @Override
        public void serialize(JsonObject object, SetCountFromConfig instance, JsonSerializationContext context) {
            super.serialize(object, instance, context);
            object.addProperty("config", instance.countConfig.name);
        }

        @Override
        public SetCountFromConfig deserialize(JsonObject object, JsonDeserializationContext context, LootItemCondition[] conditions) {
            CountConfig config = CountConfig.byName(GsonHelper.getAsString(object, "config"));
            return new SetCountFromConfig(conditions, config);
        }
    }
