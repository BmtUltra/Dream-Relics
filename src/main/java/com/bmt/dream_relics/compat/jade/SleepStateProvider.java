...
public enum SleepStateProvider implements IEntityComponentProvider, StreamServerDataProvider<EntityAccessor, Integer> {
    INSTANCE;
    ...
    public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
        ...
        if (remainingTicks != null && remainingTicks > 0) {
            ...
            Component name = Component.translatable("jade.dream_relics.sleep_state_name");
            Component s = Component.translatable("jade.potion", name, duration);
            tooltip.add(IThemeHelper.get().success(s));
        }
    }
    ...
    public @Nullable Integer streamData(EntityAccessor accessor) {
        if (accessor.getEntity() instanceof LivingEntity living && SleepStateManager.isSleeping(living)) {
            var persistentData = living.getPersistentData();
            if (persistentData.contains("DreamRelics")) {
                var sleepData = persistentData.getCompound("DreamRelics");
                int currentTicks = sleepData.getInt("SleepTicks");
                int duration = sleepData.getInt("SleepDuration");
    ...
    public ResourceLocation getUid() {
        return DRJadePlugin.SLEEP_STATE;
    }
}